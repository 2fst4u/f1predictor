...
def test_run_predictions_no_longer_bypasses_ml_when_actuals_present(sample_roster):
    """Verify that run_predictions_for_event executes ML pipeline even when actual results are available."""
    ...
    jc = MagicMock()
    # Mock schedule/event resolution - date in the far past to ensure actuals are checked
    # Use 'qualifying' key in race_info to trigger sess_dt resolution for the session
    race_info = {
        "round": "1",
        "date": "2020-03-05",
        "time": "15:00:00Z",
        "raceName": "Bahrain GP",
        "Qualifying": {"date": "2020-03-04", "time": "15:00:00Z"}
    }
    jc.get_season_schedule.return_value = [race_info]
    jc.get_latest_season_and_round.return_value = ("2020", "1")
    jc.get_event.return_value = race_info
    ...
    with patch("f1pred.predict.JolpicaClient", return_value=jc), \
        ...
        # Execute
        run_predictions_for_event(cfg, season="2020", rnd="1", sessions=["qualifying"])

        # Assertions
        mock_build_roster.assert_called()

        # ML pipeline SHOULD be called now
        mock_build_features.assert_called()
        mock_train.assert_called()

        # Actual positions SHOULD be fetched at the end of the loop
