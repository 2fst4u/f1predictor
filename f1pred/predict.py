...
from .features import build_roster, build_session_features
...
from .features import collect_historical_results
...
def run_predictions_for_event(
    ...
    for sess in sessions:
        try:
            ...
            # Check for high-level persistent cache hit (season/round/session/model_version)
            # This prevents re-calculating stats for finished sessions that are already cached.
            hl_cache_key = f"hl_{season_i}_{round_i}_{sess}_{cfg.app.model_version.replace('.', '_')}"
            if (hl_hit := pred_cache.get_by_key(hl_cache_key)):
                ranked = hl_hit["ranked"]
                prob_matrix = hl_hit["prob_matrix"]
                pairwise = hl_hit["pairwise"]
                meta = hl_hit.get("meta", {})
                logger.info(f"[predict] High-level cache hit for {season_i} R{round_i} {sess}")
            else:
                ...
                with StatusSpinner(f"Predicting {event_title} - {sess}...", on_update=progress_callback) as spinner:
                    ...
                    if (cached_hit := pred_cache.get(cache_inputs := {
                        ...
                        pred_cache.set(cache_inputs, cache_data)

                    # Also store in high-level cache
                    pred_cache.set(None, cache_data, key_override=hl_cache_key)
            ...
            if actual_positions is not None:
                ...
                if "actual_position" not in ranked.columns:
                    # If actual_positions is resolved late (from _get_actual_positions_for_session above),
                    # it is already aligned to the currently-sorted "ranked" dataframe.
                    # If it was resolved earlier (not currently possible but for future-proofing),
                    # we would need to map by driverId.
