...
## 2026-10-24 - Unbounded Input in FastAPI Endpoints
**Vulnerability:** The FastAPI endpoints `/api/schedule/{season}`, `/api/event-status/{season}/{round}`, `/api/predict`, and `/api/predict/stream` accepted string inputs for `season`, `round`, and `sessions` without enforcing maximum length constraints. An attacker could send arbitrarily long strings, potentially causing memory exhaustion, regular expression denial of service (ReDoS) downstream, or backend errors when querying external services or databases.
**Learning:** Even strongly typed frameworks like FastAPI default to unbounded inputs for basic types like `str` unless explicitly constrained using `Path(...)` or `Query(...)` validations.
**Prevention:** Always define explicit length constraints on all string parameters in public-facing API endpoints using `max_length` (e.g., `Query(..., max_length=10)`) to enforce safe boundaries and prevent resource exhaustion.
